...
##' @examples 
##' # See vignette
roc_2panel=function(rocs,labels=names(rocs),col=1:length(rocs),
...
##' @examples 
##' # See vignette
prc_2panel=function(prcs,labels=names(prcs),col=1:length(prcs),
                    highlight=NULL,yrange_lower=NULL,legend_title="") {
...
##' @param lpos legend position; as accepted by ggplot legend.position
##' @param yrange_lower y range for lower plot. If NULL, generates automatically
##' @param legend_title title for legend, defaults to nothing
##' @return Silently return ggplot object
##' @export
...
groupmetric_2panel=function(objs,labels=names(objs),col=1:length(objs),yrange=NULL,
  ...
  # Fix colours
  if (length(col)>length(objs)) col=col[1:length(objs)]
  ...
  p1 = p1 + xlab("") + ylab("Probability") +
    theme_minimal(base_size = 8) + theme(legend.justification = c(1,0),
                                         ...
                                         legend.background = 
                                           element_rect(fill = "white", linewidth=0, colour = "white"))
  if (!is.null(yrange)) p1=p1 + ylim(yrange[1],yrange[2])
  ...
  # Set up data frame for lower panel
  df2=build_diff(df,"x")
  w1=which(df2$Model==labels[1])
  bx=df2$y[w1]
  cx=df2$du[w1]-bx
  for (i in 1:length(labels)) {
    w=which(df2$Model==labels[i])
    dyb=df2$y[w]
    xci=sqrt((df2$du[w]-dyb)^2 + cx^2)
    df2$y[w]= dyb-bx
    df2$du[w]=dyb-bx + xci 
    df2$dl[w]=dyb-bx - xci
  }
  ...
  if (logscale) {
    p1=suppressMessages(p1 + scale_x_continuous(trans='log',labels = label_number(accuracy = 0.111),
                                                limits=c(0.01,0.99)) ) 
    p2=suppressMessages(p2 + scale_x_continuous(trans='log',labels = label_number(accuracy = 0.111),
                                                limits=c(0.01,0.99)) )
  } else {
    p1=suppressMessages(p1 + xlim(0,1))
    p2=suppressMessages(p2 + xlim(0,1))
  }
  ...
  # Highlights
  if (!is.null(highlight)) {
    p1 = p1 + geom_vline(xintercept=highlight,col="red",linetype="dashed")
    p2 = p2 + geom_vline(xintercept=highlight,col="red",linetype="dashed")
...
plot_decomp=function(decomp1,decomp2,threshold,labels,
  ...
  dat=dat[,setdiff(colnames(dat),c("Died.of.unrecorded","Not.recorded","Other"))]
  ...
  # Partition into 'Died of' and not
  rownames(ci)=paste0("ci_",rownames(ci))
  pdat=rbind(pdat,ci)
  pdat2=pdat[,grep("Died.of",colnames(pdat))]
  pdat1=pdat[,grep("Died.of",colnames(pdat),invert=TRUE)]
  
  for (ii in 1:2) {
    
    if (ii==1) pdat=pdat1 else pdat=pdat2
    
    # Set up data frame: survived admission
    tpdat=data.frame(t(pdat))
    tpdat=tpdat[order(-tpdat[,1]),]
    
    t2=data.frame()
    for (i in 1:5) {
      t2=rbind(t2,data.frame(Cat=rownames(tpdat),
                             group=colnames(tpdat)[i],
                             Prop=tpdat[,i],
                             upper=tpdat[,i]+tpdat[,i+5],
                             lower=tpdat[,i]-tpdat[,i+5]))
    }
    
    if (ii==1) {
      xl=unique(t2$Cat)
      t2$Cat=factor(t2$Cat,levels=xl)
    }
    if (ii==2) t2$Cat=factor(t2$Cat,levels=paste0("Died.of.",xl))
    
    assign(paste0("p",ii),
           ggplot(t2,aes(x=.data$Cat,y=.data$Prop,fill=.data$group)) + 
             geom_bar(stat="identity", position=position_dodge()) + 
             theme(axis.text.x = element_text(angle = 90,hjust=1)) +
             geom_errorbar(aes(x=.data$Cat,ymin=.data$lower,ymax=.data$upper),
                           position=position_dodge(width=0.9),width=0.4, colour="black"))
    
  }
  
  # Legend for top plot only
  p1=p1 + theme(legend.justification = c(1,0),
                legend.position = c(1,0.3),
                legend.spacing = unit(0, "npc"),
                legend.margin = margin(),
                legend.background = 
                  element_rect(fill = "white", linewidth=0, colour = "white")) +
    scale_fill_discrete(labels=llx,name="Amongst")
  p2=p2 + theme(legend.position = "none")
  
  if (inc_died) {
    p1 = p1 + xlab("") + theme(axis.text.x=element_blank(), 
                               axis.ticks.x=element_blank()) + 
      ggtitle("Admitted")
    p2 = p2 + scale_x_discrete(labels=gsub("."," ",xl,fixed=TRUE))
    p = p1 / p2 + plot_layout(heights = c(3,1)) +
      ggtitle("Died") + xlab("")
  } else {
    p1 = p1 + scale_x_discrete(labels=gsub("."," ",xl,fixed=TRUE)) + xlab("")
  }
